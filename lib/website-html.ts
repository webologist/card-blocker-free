...
(function() {
  ...
  function displaySavedCards(cardsData) {
    ...
    setTimeout(() => {
      // Look for where to insert the cards - after the "Add card to block" form
      // First try to find the button that says "Add card to block"
      let insertPoint = null;
      const buttons = Array.from(document.querySelectorAll('#root button'));

      // Find the primary action button (Add card to block or Add card)
      const addCardBtn = buttons.find(b =>
        b.textContent.includes('Add card') ||
        b.textContent.includes('add card')
      );

      if (addCardBtn) {
        // Find the form container that contains this button
        let formContainer = addCardBtn.closest('div[class*="bg-white"]') || addCardBtn.closest('div');
        if (formContainer) {
          insertPoint = formContainer.parentElement;
          console.log('[CARD-DISPLAY] Found form container, will insert after it');
        }
      }

      if (!insertPoint) {
        // Fallback: look for any content area in #root
        const root = document.getElementById('root');
      ...
      if (!insertPoint) {
        console.log('[CARD-DISPLAY] Could not find insertion point');
        return;
      }
